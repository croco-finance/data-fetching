...
describe('Test fees and fee estimate', () => {
  ...
  beforeAll(async function () {
    ...
    totalFeesFromContract = await getPositionFees(POSITION_ID, position.owner, latestIndexedBlock)
    ;[token0Price, token1Price] = await getPoolTokenPrices(position.pool, latestIndexedBlock)
    positionLiquidityUsd =
      Number(position.amount0.toSignificant()) * token0Price + Number(position.amount1.toSignificant()) * token1Price
  ...
  test('24h fee estimate multiplied by the amount of days the position exists has less then a 5% error compared to the total position fees', async () => {
    ...
    const totalFeesToken0UsdContract =
      Number(formatUnits(totalFeesFromContract.amount0, position.token0Decimals)) * token0Price
    const totalFeesToken1UsdContract =
      Number(formatUnits(totalFeesFromContract.amount1, position.token1Decimals)) * token1Price
    const totalFeesUsdContract = totalFeesToken0UsdContract + totalFeesToken1UsdContract

    const err = (Math.abs(totalFeesUsdEstimate - totalFeesUsdContract) / totalFeesUsdContract) * 100
